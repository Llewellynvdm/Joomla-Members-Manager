...
/**
 * Membersmanager Country Model
 */
class MembersmanagerModelCountry extends JModelAdmin
{    
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_membersmanager.country';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'country', $prefix = 'MembersmanagerTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_membersmanager.country');
			}
		}
	...
	public function getVvzregions()
	{
		...
		$query->join('LEFT', $db->quoteName('#__membersmanager_country', 'g') . ' ON (' . $db->quoteName('a.country') . ' = ' . $db->quoteName('g.id') . ')');
		...
		if ($db->getNumRows())
		{
			...
			if (MembersmanagerHelper::checkArray($items))
			{
				...
				{
					$access = (JFactory::getUser()->authorise('region.access', 'com_membersmanager.region.' . (int) $item->id) && JFactory::getUser()->authorise('region.access', 'com_membersmanager'));
					if (!$access)
					{
						unset($items[$nr]);
						continue;
	...
	{
		// Get the form.
		$form = $this->loadForm('com_membersmanager.country', 'country', array('control' => 'jform', 'load_data' => $loadData));
		...
		if ($id != 0 && (!$user->authorise('country.edit.state', 'com_membersmanager.country.' . (int) $id))
			|| ($id == 0 && !$user->authorise('country.edit.state', 'com_membersmanager')))
		{
		...
		if (!$user->authorise('core.edit.created_by', 'com_membersmanager'))
		{
			// Disable fields for display.
		...
		if (!$user->authorise('core.edit.created', 'com_membersmanager'))
		{
			// Disable fields for display.
	...
	{
		return 'administrator/components/com_membersmanager/models/forms/country.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('country.delete', 'com_membersmanager.country.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('country.edit.state', 'com_membersmanager.country.' . (int) $recordId);
		...
		return $user->authorise('country.edit.state', 'com_membersmanager');
	}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('country.edit', 'com_membersmanager.country.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('country.edit',  'com_membersmanager');
	}
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__membersmanager_country'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_membersmanager.edit.country.data', array());
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('country', 'membersmanagerTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_MEMBERSMANAGER_COUNTRY_SAVE_WARNING');
				}
