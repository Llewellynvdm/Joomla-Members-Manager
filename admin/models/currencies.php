 ...
 * Currencies Model
 */
class MembersmanagerModelCurrencies extends JModelList
{
	...
	{ 
		...
		if (MembersmanagerHelper::checkArray($items))
		{
			...
			{
				$access = (JFactory::getUser()->authorise('currency.access', 'com_membersmanager.currency.' . (int) $item->id) && JFactory::getUser()->authorise('currency.access', 'com_membersmanager'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__membersmanager_currency");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__membersmanager_currency'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__membersmanager_currency'))->set($fields)->where($conditions); 
