...
class MembersmanagerTableMember extends JTable
{
	...
	{
		parent::__construct('#__membersmanager_member', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_membersmanager.member'));
	}	
	...
	/**
	 * Overload the store method for the Member table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New member. A member created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('member', 'MembersmanagerTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_MEMBERSMANAGER_MEMBER_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_membersmanager.member.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_membersmanager.member.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = NULL, $id = NULL) 
	{
		...
		$asset->loadByName('com_membersmanager');
