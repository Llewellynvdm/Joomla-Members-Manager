...
abstract class MembersmanagerHelper
{
	...
	* 
	*/
	public static function allowEdit(&$item, $view, $views, $component = 'com_membersmanager')
	{
		// build record
		$record = new stdClass();
	...
	* 
	*/
	protected static function canEditItem(&$record, &$item, $view, $views, $component = 'com_membersmanager')
	{
	...
	*/
	 ...
	 *
	 */
	public static function getMemberForms($id = 0, $_component = 'com_membersmanager', $_model = 'form', $data = array(), $loadData = true, $control = null)
	{
		$form = false;
		// get APP
		...
		// get Model
		$model = $helperClass::getModel($_model, JPATH_ADMINISTRATOR . '/components/' . $_component, $Component);
		...
		if ($model)
		{
			...
			// set control if not set
			if (!$control)
			{
				$control = $component;
			}
			// get the form (handles all default permissions)
			$form = $model->getForm($data, $loadData, array('control' => $control));
		}
	...
	*/
	 ...
	 *
	 */
	public static function triggerPlugin($plugin, $event, $data)
	{
	...
	public static function onPrivacyCanRemoveData(&$plugin, &$status, &$request, &$user)
	{
		...
		if (!$user->authorise('member.delete', 'com_membersmanager') && !$user->authorise('member.privacy.delete', 'com_membersmanager'))
		{
			$reasons[] = JText::_('COM_MEMBERSMANAGER_PRIVACY_CANT_REMOVE_MEMBERS');
		}
		// Check if any reasons were found not to allow removal
		if (self::checkArray($reasons))
		{
			$status->canRemove = false;
			$status->reason = implode(' ' . PHP_EOL, $reasons) . ' ' . PHP_EOL . JText::_('COM_MEMBERSMANAGER_PRIVACY_CANT_REMOVE_CONTACT_SUPPORT');
		}
		return $status;
	}
	...
	{
		// Check if user has permission to access Members
		if ($user->authorise('member.access', 'com_membersmanager') || $user->authorise('member.privacy.access', 'com_membersmanager'))
		{
			// Get Member domain
			$domains[] = self::createMembersDomain($plugin, $user);
		}
		return $domains;
	}

	/**
	 * Create the domain for the Member
	 *
	 * @param   JTableUser  $user  The JTableUser object to process
	 *
	 * @return  PrivacyExportDomain
	 */
	protected static function createMembersDomain(&$plugin, &$user)
	{
		// create Members domain
		$domain = self::createDomain('member', 'membersmanager_member_data');
		// get database object
		$db = JFactory::getDbo();
		// get all item ids of Members that belong to this user
		$query = $db->getQuery(true)
			->select('id')
			->from($db->quoteName('#__membersmanager_member'));
		$query->where($db->quoteName('user') . ' = ' . $db->quote($user->id));
		// get all items for the Members domain
		$pks = $db->setQuery($query)->loadColumn();
		// get the Members model
		$model = self::getModel('members', JPATH_ADMINISTRATOR . '/components/com_membersmanager');
		// Get all item details of Members that belong to this user
	...
	{
		// Check if user has permission to delet Members
		if ($user->authorise('member.delete', 'com_membersmanager') || $user->authorise('member.privacy.delete', 'com_membersmanager'))
		{
			// Anonymize Member data
			self::anonymizeMembersData($plugin, $user);
		}
	}

	/**
	 * Anonymize the Member data
	 *
	 * @param   JTableUser  $user  The JTableUser object to process
	 *
	 * @return  void
	 */
	protected static function anonymizeMembersData(&$plugin, &$user)
	{
		// get database object
		$db = JFactory::getDbo();
		// get all item ids of Members that belong to this user
		$query = $db->getQuery(true)
			...
			->from($db->quoteName('#__membersmanager_member'));
		$query->where($db->quoteName('user') . ' = ' . $db->quote($user->id));
		// get all items for the Members table that belong to this user
		$pks = $db->setQuery($query)->loadColumn();

		if (self::checkArray($pks))
		{
			// get the member model
			$model = self::getModel('member', JPATH_ADMINISTRATOR . '/components/com_membersmanager');
			// this is the pseudoanonymised data array for Members
			...
			if($strict_permission_per_field)
			{
				...
				if (!$user->authorise('member.edit.name', 'com_membersmanager') || !$user->authorise('member.view.name', 'com_membersmanager'))
				{
					unset($pseudoanonymisedData['name']);
				}
				if (!$user->authorise('member.edit.email', 'com_membersmanager') || !$user->authorise('member.access.email', 'com_membersmanager') || !$user->authorise('member.view.email', 'com_membersmanager'))
				{
					unset($pseudoanonymisedData['email']);
				}
				if (!$user->authorise('member.edit.account', 'com_membersmanager') || !$user->authorise('member.view.account', 'com_membersmanager'))
				{
					unset($pseudoanonymisedData['account']);
				}
				if (!$user->authorise('member.edit.token', 'com_membersmanager') || !$user->authorise('member.view.token', 'com_membersmanager'))
				{
					unset($pseudoanonymisedData['token']);
				}
				if (!$user->authorise('member.edit.profile_image', 'com_membersmanager') || !$user->authorise('member.access.profile_image', 'com_membersmanager') || !$user->authorise('member.view.profile_image', 'com_membersmanager'))
				{
					unset($pseudoanonymisedData['profile_image']);
				}
				if (!$user->authorise('member.edit.main_member', 'com_membersmanager') || !$user->authorise('member.view.main_member', 'com_membersmanager'))
				{
					unset($pseudoanonymisedData['main_member']);
				}
				if (!$user->authorise('member.edit.password_check', 'com_membersmanager') || !$user->authorise('member.access.password_check', 'com_membersmanager') || !$user->authorise('member.view.password_check', 'com_membersmanager'))
				{
					unset($pseudoanonymisedData['password_check']);
				}
				if (!$user->authorise('member.edit.password', 'com_membersmanager') || !$user->authorise('member.access.password', 'com_membersmanager') || !$user->authorise('member.view.password', 'com_membersmanager'))
				{
					unset($pseudoanonymisedData['password']);
				}
				if (!$user->authorise('member.edit.useremail', 'com_membersmanager') || !$user->authorise('member.access.useremail', 'com_membersmanager') || !$user->authorise('member.view.useremail', 'com_membersmanager'))
				{
					unset($pseudoanonymisedData['useremail']);
				}
				if (!$user->authorise('member.edit.username', 'com_membersmanager') || !$user->authorise('member.access.username', 'com_membersmanager') || !$user->authorise('member.view.username', 'com_membersmanager'))
				{
					unset($pseudoanonymisedData['username']);
				}
				if (!$user->authorise('member.edit.surname', 'com_membersmanager') || !$user->authorise('member.view.surname', 'com_membersmanager'))
				{
					unset($pseudoanonymisedData['surname']);
				}
			}
			// get the Members table
			$table = $model->getTable();
			// check that we still have pseudoanonymised data for Members set
			...
			foreach ($pks as $i => $pk)
			{
				...
				if (isset($pseudoanonymisedData['name']))
				{
					$pseudoanonymisedData['name'] = self::randomkey(8);
				}
	...
	*/
	 ...
	 * 
	 */
	public static function getActions($view, &$record = null, $views = null, $target = null, $component = 'membersmanager', $user = 'null')
	{
	...
	public static function transliterate($string)
	{
		...
		{
			// get global value
			self::$langTag = JComponentHelper::getParams('com_membersmanager')->get('language', 'en-GB');
		}
